  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (error.code === "auth/email-already-in-use") {
        ...
        setServerError("Lỗi đăng ký: " + error.message);
      ...
      <div className="w-full max-w-sm bg-white p-4 sm:p-6 md:p-8 rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="relative">
            <input
              className="w-full border text-sm sm:text-base md:text-lg border-gray-300 rounded px-4 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500 pr-10"
              id="passInput"
          ...
          <button
            className="w-full text-sm sm:text-base md:text-lg bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded transition flex items-center justify-center cursor-pointer"
            type="submit"
            disabled={isLoading}
          >
            {isLoading ? <ClipLoader size={20} color="#ffffff" /> : "Đăng ký"}
          </button>
        </form>
